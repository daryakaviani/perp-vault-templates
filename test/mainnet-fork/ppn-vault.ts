...
describe("Mainnet Fork Tests", function () {
  ...
  this.beforeAll("send everyone usdc", async () => {
    ...
    await usdc
      .connect(signer)
      .transfer(depositor1.address, p1DepositAmount.div(1000000000000));
    await usdc
      .connect(signer)
      .transfer(depositor2.address, p2DepositAmount.div(1000000000000));
    await usdc
      .connect(signer)
      .transfer(depositor3.address, p3DepositAmount.div(1000000000000));
    await provider.send("evm_mine", []);
    await provider.send("hardhat_stopImpersonatingAccount", [usdcWhale]);
  });
